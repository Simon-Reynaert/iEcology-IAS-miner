...
def scrape_flickr_data(flickr_client, geolocator, species_list, bounding_boxes):
    """
    Main function to scrape Flickr for invasive species data.

    Args:
        flickr_client: An initialized FlickrAPI client.
        geolocator: A geopy.geocoders.Nominatim client.
        species_list: A list of scientific names to search for.
        bounding_boxes: A dictionary of countries and their bounding boxes.
    
    Returns:
        A pandas DataFrame containing the scraped results.
    """
            ...
            while True:
                try:
                    resp = flickr.photos.search(
                        ...
                        min_taken_date   = "2004-01-01", #adapt start date as needed
                        max_taken_date   = datetime.now().strftime("%Y-%m-%d"), #adapt end date as needed
                        has_geo          = 1,
...
if __name__ == "__main__":
    ...
    # Call the main function with the initialized variables
    ...
    output_file = "flickr_species_observations_eu_combined_latin_normtag_2004-now.csv"
